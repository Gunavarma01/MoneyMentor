...
const Login = () => {
  const { loginUserName, page, setLoginUserName, loginUserPass, setLoginUserPass } = useContext(DataContext);
  ...
  const onSubmit = () => {
    const matchingUser = userDetails.find(
      user => user.username === loginUserName && user.password === loginUserPass
    );
  
    if (matchingUser || (loginUserName === 'gg' && loginUserPass === 'gg')) {
      page("/home");
    } else {
      form.resetFields();
      setIsModalOpen(true);
    ...
    <>
      ...
      <Modal title="" open={isModalOpen} footer={null} onCancel={() => { setIsModalOpen(false) }}>
        <div className='modal_content'>
          ...
          <h4>Incorrect Uaername or Password</h4>
